...
class SleepStaging:
    ...
    * Zhang, Guo-Qiang, et al. "The National Sleep Research Resource: towards
    ...
    def __init__(self, data_epochs, eeg_name, *, eog_name=None, emg_name=None, metadata=None):
        # Type check
        ...
        self.data = data_epochs # (N x 3000 x C)
    ...
    def fit(self):
        ...
        for i, c in enumerate(self.ch_types):
            # Preprocessing
            epochs = np.zeros((N, 3000))
            for n in range(N):
            # - Filter the data
                epochs[n, :] = filter_data(
                    self.data[n, :, i].tolist(), sf, l_freq=freq_broad[0], h_freq=freq_broad[1], verbose=False
                )
